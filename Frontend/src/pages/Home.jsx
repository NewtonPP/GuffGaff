...
const OnlineLine = ({ online, className = "" }) => (
  <span
    className={`inline-flex items-center gap-2.5 text-[0.85rem] font-medium text-graphite-500 ${className}`}
  >
    <span className="relative grid h-2 w-2 place-items-center">
      {online !== null && online > 0 && (
        <span className="absolute h-2 w-2 animate-pulse-ring rounded-full bg-sage-500" />
      )}
      <span
    ...
    {online === null ? (
      ...
      <>
        <span className="font-bold text-graphite-800">{online}</span>
...
const Underline = ({ className = "" }) => (
  <svg
    ...
    aria-hidden="true"
    className={`absolute -bottom-[0.06em] left-0 h-[0.22em] w-full ${className}`}
  >
    <path
      d="M3 6.4c45-2.6 95-3.9 149-4 41-.1 82 .9 145 3.9"
...
const Label = ({ children, className = "" }) => (
  <span
    className={`font-display text-[0.72rem] font-extrabold uppercase tracking-[0.2em] text-sage-600 ${className}`}
  >
    {children}
  </span>
);
...
const CallCard = () => (
  <div className="relative mx-auto w-full max-w-[27rem]">
    <div className="card animate-float relative overflow-hidden p-2.5 shadow-[0_36px_70px_-34px_rgba(30,32,28,0.5)]">
      <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-graphite-950">
        {/* soft, low-key light rather than a neon mesh */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_18%,#3a4634,transparent_62%),radial-gradient(ellipse_at_50%_100%,#20241e,transparent_70%)]" />

        {/* abstract figure, lit from above */}
        <div className="absolute left-1/2 top-[24%] h-[5.5rem] w-[5.5rem] -translate-x-1/2 rounded-full bg-gradient-to-b from-sage-200/85 to-sage-500/45 blur-[1px]" />
        <div className="absolute left-1/2 top-[57%] h-36 w-52 -translate-x-1/2 rounded-[50%] bg-gradient-to-b from-sage-600/60 to-transparent blur-md" />

        <div className="grain absolute inset-0" />

        <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-md bg-graphite-950/70 px-2 py-1 font-display text-[0.66rem] font-bold uppercase tracking-[0.14em] text-sage-300">
        ...
        <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-end gap-[3px]">
          {[0, 0.18, 0.34, 0.1, 0.44, 0.26, 0.06].map((delay, i) => (
            <span
              key={i}
              className="animate-bar h-6 w-[3px] origin-bottom rounded-full bg-sand-100/85"
              style={{ animationDelay: `${delay}s` }}
            />
          ))}
        </div>

        <div className="absolute bottom-3 right-3 h-14 w-20 overflow-hidden rounded-lg border border-sand-100/20 bg-graphite-900">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_30%,#4a5540,transparent_65%)]" />
          <div className="absolute left-1/2 top-[28%] h-5 w-5 -translate-x-1/2 rounded-full bg-taupe-300/70" />
          <div className="absolute left-1/2 top-[62%] h-8 w-12 -translate-x-1/2 rounded-[50%] bg-taupe-400/40 blur-[3px]" />
        </div>
      </div>

      <div className="flex items-center justify-between px-1.5 pb-0.5 pt-2.5">
        <span className="text-[0.7rem] font-medium text-graphite-400">
          Talking for 4 minutes
        </span>
        <div className="flex gap-1.5">
          <span className="h-6 w-6 rounded-md bg-sand-200" />
          <span className="h-6 w-6 rounded-md bg-sand-200" />
          <span className="h-6 w-6 rounded-md bg-sage-600" />
        </div>
      </div>
    </div>

    {/* placed by hand, not on a grid */}
    <div className="absolute -left-4 top-[28%] -rotate-2 rounded-xl rounded-bl-sm border border-sand-300 bg-sand-50 px-3.5 py-2 text-[0.85rem] font-semibold text-graphite-700 shadow-[0_10px_24px_-14px_rgba(30,32,28,0.45)] sm:-left-12">
      where are you from?
    </div>
    <div className="absolute -right-3 bottom-[38%] rotate-1 rounded-xl rounded-br-sm bg-sage-600 px-3.5 py-2 text-[0.85rem] font-semibold text-sand-50 shadow-[0_10px_24px_-12px_rgba(60,83,51,0.7)] sm:-right-10">
      kathmandu — you?
    </div>
  </div>
);
...
const Nav = ({ onStart }) => (
  <header className="absolute inset-x-0 top-0 z-50">
    <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-6">
      ...
      <nav className="hidden items-center gap-8 text-[0.9rem] font-semibold text-graphite-600 md:flex">
        {[
          ["Why", "#why"],
          ["How it works", "#how"],
          ["Privacy", "#privacy"],
        ].map(([label, href]) => (
          <a key={href} href={href} className="transition-colors hover:text-sage-700">
            {label}
          </a>
        ))}
      </nav>
      ...
      <button
        onClick={onStart}
        className="group inline-flex items-center gap-2 font-display text-[0.9rem] font-bold text-graphite-900 transition-colors hover:text-sage-700"
      >
        Start
...
const Hero = ({ onStart, online }) => (
  <section className="relative px-6 pb-28 pt-36 sm:pt-44">
    <div className="mx-auto grid max-w-6xl items-center gap-16 lg:grid-cols-[1.1fr_0.9fr]">
      <div>
        <h1 className="animate-rise text-[clamp(2.35rem,5vw,3.7rem)] leading-[1.08]">
          Somewhere out there,
          <br />
          someone&rsquo;s{" "}
      ...
      <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[minmax(0,21rem)_1fr] lg:gap-20">
        <div className="self-start lg:sticky lg:top-24">
          <Reveal>
            ...
            <h2 className="mt-5 text-[clamp(1.75rem,3.2vw,2.4rem)] leading-[1.12]">
              Built for the good kind of small talk
            </h2>
            <p className="mt-5 leading-[1.75] text-graphite-500">
        ...
        <div>
          {points.map((point, i) => (
            ...
            >
              <span className="mt-1.5 shrink-0 font-display text-[0.95rem] font-extrabold tracking-wide text-sage-600">
                0{i + 1}
              </span>
              <div>
                <h3 className="text-[1.3rem] leading-snug">{point.title}</h3>
                <p className="mt-3 max-w-xl leading-[1.75] text-graphite-500">{point.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="mt-16 grid gap-10 sm:grid-cols-3 sm:gap-8">
          {steps.map((step, i) => (
            <Reveal key={step.title} delay={i * 90} className="rule pt-7">
              <span className="font-display text-[2.6rem] font-extrabold leading-none tracking-[-0.04em] text-sage-500">
                {i + 1}
              </span>
              <h3 className="mt-5 text-[1.25rem]">{step.title}</h3>
              <p className="mt-3 leading-[1.75] text-graphite-500">{step.body}</p>
            </Reveal>
          ))}
        </div>
      </div>
  ...
  <section id="privacy" className="relative scroll-mt-24 px-6 py-24">
    <div className="mx-auto max-w-6xl">
      <Reveal className="card grain relative overflow-hidden px-8 py-14 sm:px-14">
        <div className="wash -left-20 top-0 h-72 w-96 bg-sage-300/45" />

        <div className="relative grid gap-12 lg:grid-cols-[1fr_1fr] lg:gap-20">
          <div>
            <Label>Privacy</Label>
            <h2 className="mt-5 text-[clamp(1.75rem,3.2vw,2.35rem)] leading-[1.12]">
              There is nothing here to leak
            </h2>
            <p className="mt-5 max-w-md leading-[1.75] text-graphite-600">
          ...
          <ul className="self-center">
            {[
              ...
              <li key={title} className={`py-5 ${i > 0 ? "rule" : ""}`}>
                <p className="font-display font-bold text-graphite-900">{title}</p>
                <p className="mt-1.5 text-[0.92rem] leading-relaxed text-graphite-500">{body}</p>
              </li>
            ))}
          </ul>
        </div>
...
const FinalCta = ({ onStart, online }) => (
  <section className="relative px-6 pb-28 pt-10">
    <Reveal className="mx-auto max-w-2xl text-center">
      <h2 className="text-[clamp(1.9rem,4vw,2.7rem)] leading-[1.12]">
        Someone pressed start
        <br />a second before you did.
      </h2>
      <p className="mx-auto mt-6 max-w-sm leading-[1.75] text-graphite-500">
  ...
  <footer className="rule relative px-6 py-10">
    <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-5 text-[0.88rem] font-medium text-graphite-400 sm:flex-row">
      ...
      <nav className="flex gap-6 font-semibold">
        ...
        <a href="#privacy" className="transition-colors hover:text-sage-700">Privacy</a>
      </nav>
    </div>
  </footer>
);
...
const Home = () => {
  ...
  return (
    <div className="grain relative min-h-screen overflow-x-clip bg-sand-100">
      {/* two soft washes behind the fold, and nothing else */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[46rem] overflow-hidden" aria-hidden="true">
        <div className="wash -top-56 left-[-10%] h-[34rem] w-[46rem] bg-sage-300/50" />
        <div className="wash -top-32 right-[-5%] h-[26rem] w-[30rem] bg-taupe-300/40" />
      </div>
